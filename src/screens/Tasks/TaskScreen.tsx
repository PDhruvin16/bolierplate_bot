...
const TaskScreen: React.FC<TaskScreenProps> = ({ navigation }) => {
  ...
  const filteredTasks = tasks.filter(
    task =>
      ...
      task.email.toLowerCase().includes(searchText.toLowerCase()) ||
      task.city.toLowerCase().includes(searchText.toLowerCase()),
  );
  const themedStyles = {
    container: {
      ...styles.container,
  ...
  const handleAccountPress = useCallback(
    (account: TaskItem) => {
      if (isSelectionMode) {
        const isSelected = selectedTasks.includes(account.id);
        if (isSelected) {
          setSelectedTasks(prev => prev.filter(id => id !== account.id));
        } else {
          setSelectedTasks(prev => [...prev, account.id]);
        }
      } else {
  ...
  const handleAccountMenu = useCallback((account: TaskItem) => {
  ...
  const handleEditAccounts = useCallback(() => {
    log.debug('Edit selected accounts:', selectedTasks);
  }, [selectedTasks]);
  ...
  const handleDeleteAccounts = useCallback(() => {
    if (selectedTasks.length > 0) {
      Alert.alert(
        'Delete Accounts',
        `Are you sure you want to delete ${selectedTasks.length} account(s)?`,
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Delete',
            ...
            onPress: async () => {
              try {
                ...
                log.debug('Tasks deleted successfully');
              } catch (error) {
  ...
  const singleSelected = selectedTasks.length === 1;
  const multiSelected = selectedTasks.length > 1;
  ...
  return (
    <View style={themedStyles.container}>
      ...
      <CustomHeader
        variant={{
          type: 'basic',
          title: isSelectionMode ? 'Task Select' : 'Task',
        }}
        onSearch={() => {}}
      ...
      <View style={themedStyles.listContainer}>
        <ScrollView
          ...
          bounces={false}
        >
          {filteredTasks.map((account, idx, arr) =>
