...
const ProductScreen: React.FC<ProductProps> = ({ navigation }) => {
  const [products, setProducts] = useState<ProductItem[]>(PRODUCT_DATA);
  ...
  const filteredProducts = products.filter(
    productItem =>
      productItem.name.toLowerCase().includes(searchText.toLowerCase()) ||
      productItem.email.toLowerCase().includes(searchText.toLowerCase()) ||
      productItem.city.toLowerCase().includes(searchText.toLowerCase()),
  );
  const themedStyles = {
    container: {
      ...styles.container,
  ...
  const handleAccountPress = useCallback(
    (productItem: ProductItem) => {
      if (isSelectionMode) {
        const isSelected = selectedProducts.includes(productItem.id);
        if (isSelected) {
          setSelectedProducts(prev => prev.filter(id => id !== productItem.id));
        } else {
          setSelectedProducts(prev => [...prev, productItem.id]);
        }
      } else {
        // Navigate to account details
        log.debug('Navigate to account details:', productItem.id);
      }
    },
    [isSelectionMode, selectedProducts],
  );

  const handleAccountMenu = useCallback((account: ProductItem) => {
  ...
  const handleNewAccount = useCallback(() => {
    navigation.navigate('NewProductScreen');
    log.debug('Create new account');
  }, []);

  const handleEditAccounts = useCallback(() => {
    log.debug('Edit selected accounts:', selectedProducts);
  }, [selectedProducts]);
  ...
  const handleDeleteAccounts = useCallback(() => {
    if (selectedProducts.length > 0) {
      Alert.alert(
        'Delete Accounts',
        `Are you sure you want to delete ${selectedProducts.length} account(s)?`,
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Delete',
            ...
            onPress: async () => {
              try {
                ...
                for (const productId of selectedProducts) {
                  await deleteAccount([productId]);
                }
                // Update local state
                setProducts(prev =>
                  prev.filter(
                    productItem => !selectedProducts.includes(productItem.id),
                  ),
                );
                setSelectedProducts([]);
  ...
  }, [selectedProducts, deleteAccount]);
  ...
  const singleSelected = selectedProducts.length === 1;
  const multiSelected = selectedProducts.length > 1;
    ...
    ...(singleSelected
      ? [
          {
            id: 'active',
            label: 'Active',
            icon: renderLogo(icons.ic_Activate, { width: 18, height: 18 }),
      ...
      ? [
          ...
          },
          {
            id: 'dactive',
            label: 'Dactive',
            icon: renderLogo(icons.ic_Deactivate, { width: 18, height: 18 }),
      ...
      : [
          {
            id: 'add_group',
            label: 'Add Group',
            icon: renderLogo(icons.add_group, { width: 18, height: 18 }),
          ...
          },
          {
            id: 'add_product',
            label: 'Add Product',
            icon: renderLogo(icons.Add_Product, { width: 18, height: 18 }),
          ...
          },
          {
            id: 'add_bundle',
            label: 'Add Bundle',
            icon: renderLogo(icons.Add_Bundle, { width: 18, height: 18 }),
  ...
  return (
    <View style={themedStyles.container}>
      ...
      <CustomHeader
        variant={{
          type: 'basic',
          title: isSelectionMode ? 'Product Select' : 'Product',
        }}
        onSearch={() => {}}
      ...
      <View style={themedStyles.listContainer}>
        <ScrollView
          ...
          bounces={false}
        >
          {filteredProducts.map((product, idx, arr) =>
            renderProductItem(product, idx, arr),
          )}
        </ScrollView>
      </View>
