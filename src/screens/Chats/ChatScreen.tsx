...
// import { PHONE_CALL_DATA, PhoneCallItem,getActionBarItems } from '../../constants/phonecallData';
import {
  CHAT_DATA,
  ChatItem,
  getActionBarItems,
...
const ChatScreen: React.FC<ChatProps> = ({ navigation }) => {
  const [chats, setChats] = useState<ChatItem[]>(CHAT_DATA);
  ...
  const filteredChats = chats.filter(
    chat =>
      ...
      chat.city.toLowerCase().includes(searchText.toLowerCase()),
  );
  const themedStyles = {
    container: {
      ...styles.container,
  ...
  const handleAccountPress = useCallback(
    (account: ChatItem) => {
      if (isSelectionMode) {
        const isSelected = selectedChats.includes(account.id);
        if (isSelected) {
          setSelectedChats(prev => prev.filter(id => id !== account.id));
        } else {
          setSelectedChats(prev => [...prev, account.id]);
        }
      } else {
    ...
    [isSelectionMode, selectedChats],
  );

  const handleAccountMenu = useCallback((account: ChatItem) => {
  ...
  const handleEditAccounts = useCallback(() => {
    log.debug('Edit selected accounts:', selectedChats);
  }, [selectedChats]);
  ...
  const handleDeleteAccounts = useCallback(() => {
    if (selectedChats.length > 0) {
      Alert.alert(
        'Delete Accounts',
        `Are you sure you want to delete ${selectedChats.length} account(s)?`,
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Delete',
            ...
            onPress: async () => {
              try {
                ...
                for (const chatId of selectedChats) {
                  await deleteAccount([chatId]);
                }
                // Update local state
                setChats(prev =>
                  prev.filter(task => !selectedChats.includes(task.id)),
                );
                setSelectedChats([]);
  ...
  const singleSelected = selectedChats.length === 1;
  const multiSelected = selectedChats.length > 1;
  ...
  return (
    <View style={themedStyles.container}>
      ...
      <CustomHeader
        variant={{
          type: 'basic',
          title: isSelectionMode ? 'Chat Select' : 'Chat',
        }}
        onSearch={() => {}}
      ...
      <View style={themedStyles.listContainer}>
        <ScrollView
          ...
          bounces={false}
        >
          {filteredChats.map((account, idx, arr) =>
