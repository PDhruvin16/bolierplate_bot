...
// import { QUEUES_DATA, QueuesItem ,getActionBarItems} from '../../constants/QueuesData';
...
const KnowledgeArticleScreen: React.FC<KnowledgeArticleProps> = ({
  navigation,
}) => {
  const [knowledgeArticles, setKnowledgeArticles] =
    useState<KnowledgeItem[]>(KNOWLEDGE_DATA);
  ...
  const filteredArticles = knowledgeArticles.filter(
    article =>
      ...
      article.email.toLowerCase().includes(searchText.toLowerCase()) ||
      article.city.toLowerCase().includes(searchText.toLowerCase()),
  );
  const themedStyles = {
    container: {
      ...styles.container,
  ...
  const handleAccountPress = useCallback(
    (article: KnowledgeItem) => {
      if (isSelectionMode) {
        ...
        log.debug('Navigate to account details:', article.id);
      }
    },
    [isSelectionMode, selectedArticles],
  );

  const handleAccountMenu = useCallback((account: KnowledgeItem) => {
  ...
  const handleNewAccount = useCallback(() => {
    navigation.navigate('NewKnowledgeArticleScreen');
    log.debug('Create new account');
  }, []);

  const handleEditAccounts = useCallback(() => {
    log.debug('Edit selected accounts:', selectedArticles);
  }, [selectedArticles]);
  ...
  const handleDeleteAccounts = useCallback(() => {
    if (selectedArticles.length > 0) {
      Alert.alert(
        'Delete Accounts',
        `Are you sure you want to delete ${selectedArticles.length} account(s)?`,
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Delete',
            ...
            onPress: async () => {
              try {
                ...
                for (const articleId of selectedArticles) {
                  await deleteAccount([articleId]);
                }
                // Update local state
                setKnowledgeArticles(prev =>
                  prev.filter(
                    articleItem => !selectedArticles.includes(articleItem.id),
                  ),
                );
                setSelectedArticles([]);
                setIsSelectionMode(false);
                log.debug('Articles deleted successfully');
              } catch (error) {
                log.error('Error deleting articles:', error);
                Alert.alert(
                  'Error',
  ...
  }, [selectedArticles, deleteAccount]);
  ...
  const singleSelected = selectedArticles.length === 1;
  const multiSelected = selectedArticles.length > 1;
  ...
  const moreActionItems: ActionItem[] = [
    ...(singleSelected
      ? [
          ...
          {
            id: 'create_major_version' ,
            label: 'Create Major Version',
                       icon: renderLogo(icons.ic_Create_major_version, { width: 18, height: 18 }),
           
            onPress: () => {
              log.debug('Create Major Version');
            },
          },
        {
            id: 'create_minor_version' ,
            label: 'Create Minor Version',
                       icon: renderLogo(icons.ic_Create_minor_version, { width: 18, height: 18 }),
           
            onPress: () => {
              log.debug('Create Minor Version');
            },
          },
          {
            id: 'export_excel',
          ...
          },
          {
            ...
            onPress: () => {
              log.debug('send to trash');
            },
          },
          {
            id: 'approve',
            label: 'Approve',
           icon: renderLogo(icons.ic_Approve, { width: 18, height: 18 }),
            onPress: () => {
              log.debug('Approve');
            },
          },
            ...
            id: 'publish',
            label: 'Publish',
                       icon: renderLogo(icons.ic_publish, { width: 18, height: 18 }),
          ...
          },
           {
            ...
            label: 'Revert to Draft',
                      icon: renderLogo(icons.ic_Revert_to_draft, { width: 18, height: 18 }),

            onPress: () => {
              log.debug('revert to draft');
            },
          },
        ]
      : multiSelected
  ...
  const renderArticleItem = (
    account: KnowledgeItem,
    index: number,
    arr: KnowledgeItem[],
  ) => {
    const isSelected = selectedArticles.includes(account.id);
  ...
  return (
    <View style={themedStyles.container}>
      ...
      <CustomHeader
        variant={{
          type: 'basic',
          title: isSelectionMode ? 'Article Select' : 'Knowledge Articles',
        }}
        onSearch={() => {}}
      ...
      <View style={themedStyles.listContainer}>
        <ScrollView
          ...
          bounces={false}
        >
          {filteredArticles.map((article, idx, arr) =>
            renderArticleItem(article, idx, arr),
          )}
        </ScrollView>
      </View>
...
export default KnowledgeArticleScreen;
